...
- [x] Experiment #0.1: Baseline 모듈화
  - 결과: 46.9526점 (+0.01점)
  - 날짜: 2025-10-13
  - 상태: ✅ 성공
  - 비고: 7개 모듈 생성, 재사용 가능한 구조
...
- [x] Phase 1.5: EDA 분석 (5개 agents 병렬 조사)
  - 날짜: 2025-10-13
  - 결과: `tasks/eda-findings.md` 문서화 완료
  - 상태: ✅ 완료
  - 주요 발견:
    1. 후처리 개선 (즉시 적용) → +0.5~1.2점
    2. Learning Rate 튜닝 (최우선) → +1~2점
    3. Special Token 추가 (Time/Money) → +0.5~1.5점
    4. 데이터 증강 재시도 (스타일 보존) → +1~2점

- [x] Experiment #2: 후처리 개선 (Post-processing v2)
  - 결과: 46.9824점 (+0.03점, 실질적 변화 없음)
  - 날짜: 2025-10-13
  - 상태: ❌ 실패 (Baseline으로 롤백)
  - 비고: 공백 정규화 + 중복 제거, 효과 미미

- [x] Experiment #3: Learning Rate 2e-5
  - v1 (checkpoint-1750): 46.6919점 (-0.26점)
  - v2 (checkpoint-1000): 46.6089점 (-0.34점)
  - 날짜: 2025-10-13
  - 상태: ❌ 실패 (모든 checkpoint 일관된 하락)
  - 비고: LR 2e-5가 과도함, Baseline LR 1e-5가 이미 최적

---

### Planned Experiments

#### ~~Exp #2: 후처리 개선 (Post-processing v2)~~ ❌ 완료 (실패)
- **결과**: 46.9824점 (+0.03점, 실질적 변화 없음)
- **Status**: ❌ Failed - Completed Experiments 참조

#### ~~Exp #3: Learning Rate 2e-5~~ ❌ 완료 (실패)
- **결과**: v1 46.6919점 (-0.26), v2 46.6089점 (-0.34)
- **Status**: ❌ Failed - Completed Experiments 참조
- **교훈**: LR 2e-5가 과도함, Baseline LR 1e-5가 이미 최적

#### ~~Exp #4: Learning Rate 3e-5~~ ❌ Skipped
- **이유**: Exp #3 실패로 LR 증가 방향 자체가 잘못됨
- **Status**: ❌ Skipped (LR 방향 잘못됨)

#### ~~Exp #5: Learning Rate 5e-5~~ ❌ Skipped
- **이유**: Exp #3 실패로 LR 증가 방향 자체가 잘못됨
- **Status**: ❌ Skipped (LR 방향 잘못됨)

#### Exp #6: Special Token 추가 - Time
- **목표**: +0.5~1점
- **방법**:
  - 시간 표현 (`5시 30분`, `내일`, `3개월`) → `#Time#` 토큰으로 치환
  - 데이터 재전처리 (train, dev, test)
  - Tokenizer 재설정 및 재학습
- **예상 점수**: 50~51
- **소요 시간**: 3-4시간 (전처리 3시간, 학습 30분)
- **리스크**: ⚠️ Medium (재전처리 복잡도)
- **우선순위**: Day 4-5
- **Status**: 📋 Planned (2025-10-13)
- **비고**: Agent 5 권장, 데이터 빈도 15-20%

#### Exp #7: Special Token 추가 - Money
- **목표**: +0.3~0.7점
- **방법**: 금액 표현 (`50만원`, `$100`) → `#Money#` 토큰으로 치환
- **예상 점수**: 50.5~51.5
- **소요 시간**: 2시간
- **리스크**: ⚠️ Medium
- **우선순위**: Day 5-6
- **Status**: 📋 Planned (2025-10-13)
- **비고**: Agent 5 권장, 데이터 빈도 10-15%

#### Exp #8: Warmup Steps 조정
- **목표**: +0.5~1점
- **방법**:
  - `warmup_steps: 20 → 50` (Option 1)
  - `warmup_steps: 20 → 100` (Option 2)
- **예상 점수**: 51~52
- **소요 시간**: 30분
- **리스크**: ✅ Low
- **우선순위**: Week 2
- **Status**: 📋 Planned (2025-10-13)
- **비고**: Agent 2 권장, 안정적 학습 유도
...
#### Exp #10: 데이터 증강 재시도 (필터링)
- **목표**: +0.5~1점
- **방법**:
  - Exp #1 증강 데이터 중 스타일 일치 샘플만 선별
  - 스타일 분류기로 필터링 (threshold=0.8)
  - 원본 : 증강 = 2 : 1 비율 유지
- **예상 점수**: 52~53
- **소요 시간**: 2시간
- **리스크**: ⚠️ Medium
- **우선순위**: Week 2+ (선택적)
- **Status**: 📋 Planned (2025-10-13)
- **비고**: Agent 3 권장, Exp #1 실패 개선

#### Exp #11: 데이터 증강 재시도 (LLM 스타일 보존)
- **목표**: +1~2점
- **방법**:
  - LLM 프롬프트에 원본 스타일 예시 포함
  - "번역투 허용, 간결하게, 핵심만" 명시
  - 소량 테스트 (1,000개) → 검증 → 전체 적용
- **예상 점수**: 53~55
- **소요 시간**: 1-2일
- **리스크**: ⚠️ Medium
- **우선순위**: Week 2+ (선택적)
- **Status**: 📋 Planned (2025-10-13)
- **비고**: Agent 3 권장, 스타일 일관성 보존
