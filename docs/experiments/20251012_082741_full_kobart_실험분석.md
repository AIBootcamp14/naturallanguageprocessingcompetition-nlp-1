...
graph TB
    subgraph Input["입력"]
        A[--models all<br/>6개 모델 자동 확장]
        B[학습 데이터<br/>12,457개]
        C[검증 데이터<br/>499개]
    end

    subgraph Models["모델 계층"]
        D[KoBART<br/>Encoder-Decoder]
        E[Llama-3.2-3B<br/>Causal LM]
        F[Qwen3-4B<br/>Causal LM]
        G[Solar-10.7B<br/>Causal LM]
        H[Polyglot-12.8B<br/>Causal LM]
        I[KULLM-v2<br/>Causal LM]
    end

    subgraph Training["학습 계층"]
        J[K-Fold CV<br/>5-Fold]
        K[Data Augmentation<br/>4가지 방법]
        L[TTA<br/>4가지 전략]
    end

    subgraph Ensemble["앙상블"]
        M[Stacking<br/>Meta-learner]
    end

    subgraph Output["출력"]
        N[최종 제출 파일<br/>submission.csv]
    end
    ...
    style N fill:#aed581,stroke:#2e7d32,color:#000
```
...
```mermaid
graph LR
    A[6개 모델<br/>시작] --> B{KoBART<br/>학습}
    B -->|성공<br/>10 Epoch| C[✅ KoBART<br/>완료]
    B --> D{Llama-3.2-3B<br/>학습}
    D -->|실패<br/>BF16 오류| E[❌ 오류 발생<br/>나머지 5개 중단]

    style A fill:#e3f2fd,stroke:#1976d2,color:#000
    style B fill:#fff9c4,stroke:#f57f17,color:#000
    style C fill:#c8e6c9,stroke:#2e7d32,color:#000
    style D fill:#ffccbc,stroke:#d84315,color:#000
    style E fill:#ffcdd2,stroke:#c62828,color:#fff
```
...
```mermaid
graph LR
    A[초기 Loss<br/>2.5852] --> B[Epoch 1<br/>1.3194<br/>49% 감소]
    B --> C[Epoch 5<br/>약 1.0<br/>추정]
    C --> D[Epoch 10<br/>0.8411<br/>67.5% 감소]
...
```mermaid
graph TB
    subgraph Initial["초기 학습 (Epoch 0-1)"]
        A1[Loss: 2.59 → 1.32<br/>감소율: 49%]
        A2[Grad Norm: 4.3 ~ 8.8<br/>안정적]
        A3[LR: Warmup 완료<br/>10e-6]
    end

    subgraph Middle["중기 학습 (Epoch 1-5)"]
        B1[Loss: 1.32 → 1.0<br/>안정적 감소]
        B2[Grad Norm: 안정<br/>변동 적음]
        B3[LR: 선형 감소<br/>진행 중]
    end

    subgraph Final["후기 학습 (Epoch 5-10)"]
        C1[Loss: 1.0 → 0.84<br/>수렴 단계]
        C2[Grad Norm: 4.1 ~ 6.8<br/>매우 안정]
        C3[LR: 5.37e-08<br/>거의 0]
    end

    Initial --> Middle --> Final

    style Initial fill:#ffccbc,stroke:#d84315,color:#000
    style Middle fill:#fff9c4,stroke:#f57f17,color:#000
    style Final fill:#c8e6c9,stroke:#2e7d32,color:#000
```

### 3.2 KoBART 모델 학습 진행 상황
...
```mermaid
graph TB
    A[Llama-3.2-3B<br/>모델 로드 시작] --> B{Config 설정<br/>확인}
    B --> C[bf16: true<br/>torch_dtype: bfloat16]
    C --> D[모델을 BFloat16으로<br/>로드]
    D --> E{AMP GradScaler<br/>호출}
    E --> F[_amp_foreach_non_finite<br/>_check_and_unscale_cuda]
    F --> G[❌ RuntimeError<br/>BFloat16 미지원]
...
graph LR
    subgraph Root["근본 원인"]
        A1[Config 설정<br/>bf16: true]
        A2[PyTorch AMP<br/>BF16 미지원]
    end

    subgraph Direct["직접 원인"]
        B1[GradScaler<br/>CUDA 커널 미구현]
        B2[_amp_foreach_<br/>non_finite_check]
    end
    ...
    style Root fill:#ffccbc,stroke:#d84315,color:#000
    style Direct fill:#fff9c4,stroke:#f57f17,color:#000
    style Effect fill:#ffcdd2,stroke:#c62828,color:#fff
```
...
graph TB
    subgraph P0["P0 - Critical (즉시)"]
        A1[BFloat16 → FP16 변경<br/>5개 Config 파일]
        A2[영향도: HIGH<br/>작업량: LOW]
    end

    subgraph P1["P1 - High (1주일)"]
        B1[모델별 오류 처리 강화<br/>full_pipeline.py]
        B2[영향도: HIGH<br/>작업량: MEDIUM]
    end

    subgraph P2["P2 - Medium (2주일)"]
        C1[Config 검증 로직 추가<br/>config_validator.py]
        C2[영향도: MEDIUM<br/>작업량: MEDIUM]
    end

    subgraph P3["P3 - Low (1개월)"]
        D1[실험 자동 재개 기능<br/>checkpoint_manager.py]
...
```mermaid
graph TB
    A[1단계<br/>Config 수정 검증] --> B{검증<br/>통과?}
    B -->|Yes| C[2단계<br/>단일 모델 테스트]
    B -->|No| A1[Config 수정<br/>재작업]
    A1 --> A

    C --> D{Llama<br/>학습 성공?}
    D -->|Yes| E[3단계<br/>다중 모델 테스트]
    D -->|No| C1[Config 재확인<br/>디버깅]
    C1 --> C

    E --> F{2개 모델<br/>모두 성공?}
    F -->|Yes| G[4단계<br/>전체 파이프라인]
    F -->|No| E1[오류 처리<br/>강화 적용]
    E1 --> E

    G --> H{6개 모델<br/>+ 앙상블 성공?}
    H -->|Yes| I[✅ 검증 완료]
    H -->|No| G1[파이프라인<br/>재검토]
    G1 --> G
