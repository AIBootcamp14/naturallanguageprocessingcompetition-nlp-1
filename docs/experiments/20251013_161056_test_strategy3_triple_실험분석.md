...
gantt
    ...
    KoBART 학습 (완료)    :done, kobart, 16:11:01, 16:12:41

    section Model 2: Llama
    Llama 학습 (완료)     :done, llama, 16:13:23, 18:02:39

    section Model 3: Qwen
    Qwen 학습 (완료)   :done, qwen, 18:21:14, 03:46:53
```
...
| **Qwen3-4B** | ✅ 완료 | 9시간 26분 (33,939초) | gradient_accumulation_steps: 10 |
...
example_log_interpretation = {
    ...
    'example_3': {
        ...
        'interpretation': {
            ...
            'learning_rate': '5.96e-06 → Warmup 중',
            'epoch': '0.6 → 60% 학습 완료',
...
graph LR
    ...
    C --> D[최종<br/>Loss: 0.79<br/>Epoch: 2.0<br/>✅ 완료]
...
qwen_training_metrics_summary = [
    ...
    # ---------------------- 후기 (완료) ---------------------- #
    {'epoch': 1.48, 'loss': 0.8612, 'grad_norm': 1.299, 'lr': 1.476e-05}, # 계속 감소
    {'epoch': 2.0,  'loss': 0.7881, 'grad_norm': 1.419, 'lr': 1.996e-05}  # ✅ 최종 완료
]
...
qwen_eval_metrics = [
    ...
    {
        'epoch': 2.0,
        'eval_loss': 0.8513,        # ⬇️ 감소 (개선)
        'eval_rouge1': 0.1450,      # ⬆️ 약간 상승 (14.50%)
        'eval_rouge2': 0.0980,      # ⬆️ 약간 상승 (9.80%)
        'eval_rougeL': 0.1450,      # ⬆️ 약간 상승 (14.50%)
        'eval_rouge_sum': 0.3880,   # ⬆️ 약간 개선
        'eval_runtime': 3749.48,    # 1시간 2분 (여전히 느림)
        'status': '✅ 완료, 약간 개선'
    }
]
...
# ==================== Qwen 최종 결과 ==================== #
qwen_final_summary = {
    'train_runtime': 33939,                  # 9시간 26분 (33,939초)
    'train_samples_per_second': 0.147,      # 초당 0.147개 샘플 (매우 느림)
    'train_steps_per_second': 0.015,        # 초당 0.015 스텝
    'train_loss': 1.1508,                   # 평균 Train Loss
    'best_train_loss': 0.7881,              # 최저 Train Loss (Epoch 2.0)
    'best_eval_rouge_sum': 0.3880,          # 최고 ROUGE Sum (Epoch 2)
    'final_eval_rouge_sum': 0.3880,         # 최종 ROUGE Sum (Epoch 2)
    'status': '✅ 학습 완료'
}
```
...
graph TB
    subgraph Positive["긍정적 측면"]
        A1[Train Loss<br/>2.69 → 0.79<br/>71% 감소<br/>✅ 매우 우수]
        A2[Gradient Norm<br/>0.9 ~ 1.4<br/>✅ 매우 안정]
        A3[수렴 속도<br/>Epoch 0.6에<br/>Loss < 0.9<br/>✅ 매우 빠름]
    end

    subgraph Negative["부정적 측면"]
        B1[학습 시간<br/>9시간 26분<br/>❌ 과도하게 느림]
        B2[평가 시간<br/>1시간 이상<br/>❌ 매우 느림]
        B3[ROUGE 점수<br/>0.3880<br/>⚠️ 낮음]
    end
...
- ✅ **Train Loss 매우 우수**: 2.69 → 0.79 (71% 감소, 가장 빠른 수렴)
- ✅ **Gradient Norm 매우 안정**: 0.9~1.4 범위 (3개 모델 중 최고)
- ✅ **빠른 수렴 속도**: Epoch 0.6에 이미 Loss < 0.9 달성
- ✅ **Epoch 2 평가 개선**: ROUGE Sum 0.383 → 0.388 (약간 상승)
- ❌ **학습 시간 극도로 느림**: 9시간 26분 (KoBART의 342배)
- ❌ **평가 시간 과도**: 1시간 이상 소요
- ⚠️ **ROUGE 점수 낮음**: 0.3880 (KoBART의 37% 수준)

**문제 원인:**
- **치명적**: Config 파일의 `gradient_accumulation_steps: 10` 설정
- Effective batch size 60 (6×10) → Steps 수가 833개로 폭증
...
training_time_comparison = {
    ...
    'qwen3-4b': {
        'total_time_seconds': 33939,
        'total_time_formatted': '9시간 26분',
        'samples_per_second': 0.147,
        'speed_relative_to_kobart': '342x (342배 느림)',
        'status': '❌ 극도로 느림'
    }
}
```
...
| **Qwen** | 9시간 26분 | **342x 느림** | 0.147 | ❌ 극도로 느림 |
...
rouge_performance_comparison = {
    ...
    'qwen3-4b': {
        ...
        'eval_rouge2': 0.0980,
        'eval_rougeL': 0.1450,
        'eval_rouge_sum': 0.3880,
        'performance_level': '⚠️ 낮음',
        'relative_to_kobart': '37.0% (1/3 수준)'
    }
}
```
...
```mermaid
graph TB
    A[KoBART<br/>ROUGE Sum: 1.048<br/>✅ 최고 성능] --> B[Qwen<br/>ROUGE Sum: 0.388<br/>37% 수준]
    A --> C[Llama<br/>ROUGE Sum: 0.293<br/>28% 수준]
...
| **Qwen** | 14.50% | 9.80% | 14.50% | **0.3880** | ⚠️ 37.0% |
...
|------|----------|----------|--------|--------|----------|
| **Qwen** | 2.6867 | 0.7881 | -1.899 | **70.7%** | ✅ 매우 빠름 |
...
key_findings = {
    ...
    'finding_2': {
        ...
        'description': [
            ...
            'Qwen (4B): ROUGE Sum 0.388 (KoBART의 37%)'
        ],
        'possible_reasons': [
    ...
    'finding_4': {
        ...
        'severity': '⚠️ High',
        'description': [
            'Train Loss: 2.69 → 0.79 (70.7% 감소, 최고)',
            'Gradient Norm: 0.9~1.4 (매우 안정, 최고)',
            '하지만 학습 시간: 9시간 26분 (KoBART의 342배)'
        ],
        'trade_off': {
        ...
        'recommendation': [
            ...
            '재실험: 수정 후 학습 시간이 1시간 이내로 단축될 것으로 예상'
        ]
    },
...
metrics_insights = {
    ...
    'loss_rouge_correlation': {
        ...
        'data': [
            'Qwen: Train Loss 0.79 (최저) → ROUGE Sum 0.39 (중간)',
    ...
    'grad_norm_importance': {
        ...
        'data': [
            'Qwen: Grad Norm 0.9~1.4 → 안정적 학습, 빠른 수렴',
...
p0_critical_fixes = [
    # ---------------------- 1. Config 파일 수정 (최우선) ---------------------- #
    {
        ...
        'expected_improvement': {
            ...
            'qwen': '학습 시간 1/10 단축 (9시간 26분 → 56분)'
        },
        'action_items': [
...
p1_high_priority = [
    # ---------------------- 1. Causal LM 프롬프트 엔지니어링 ---------------------- #
    {
        ...
        'problem': 'Causal LM 모델의 ROUGE 점수가 매우 낮음 (0.29, 0.39)',
...
p2_medium_priority = [
    ...
    # ---------------------- 2. Full Fine-tuning 실험 ---------------------- #
    {
        ...
        'expected_improvement': 'ROUGE Sum 20~30% 향상',
        'estimated_effort': '2시간',
...
experiment_summary = {
    ...
    'successes': [
        ...
        '✅ Qwen 모델 학습 완료 (Loss 2.69 → 0.79, 70.7% 감소)',
    ...
    'failures': [
        ...
        '❌ Llama와 Qwen의 학습 시간 과도 (66배, 342배)',
        '❌ Causal LM 모델의 ROUGE 점수 매우 낮음 (0.29, 0.39)',
    ...
    # ---------------------- 앞으로의 방향 ---------------------- #
    'future_direction': [
        '🎯 즉시: Config 파일 수정 후 재실험 (학습 시간 대폭 단축)',
...
| **Qwen** | ✅ 완료, 시간 극도로 느림 | gradient_accumulation_steps: 1<br/>프롬프트 템플릿 적용<br/>Epoch 5~10으로 증가 | 학습 시간 1/10 단축<br/>ROUGE Sum 0.39 → 0.7~0.9 |
...
2. ⚠️ **Config 파일 문제 심각**: gradient_accumulation_steps 설정으로 학습 시간 66~342배 증가
3. 🔧 **즉시 조치 필요**: Config 파일 수정만으로도 극적인 개선
...
qwen_representative_log = [
    ...
    {'timestamp': '23:14:20', 'epoch': 1.04, 'loss': 0.8357, 'grad_norm': 0.930, 'lr': 1.036e-05},
...
**문서 작성 완료**: 2025-10-14 03:47
**실험 총 소요 시간**: 11시간 36분 (16:10:56 ~ 03:46:53)
**문서 업데이트**: 2025-10-14 (Qwen3-4B 완료 후 최종 업데이트)
