...
# ---------------------- 표준 요약 (150 토큰) ---------------------- #
# 일반적인 요약 길이로 균형잡힌 설정
python scripts/train.py --max_new_tokens 150

# ---------------------- 긴 요약 (200 토큰, 권장) ---------------------- #
# 한국어 토큰 특성을 고려한 충분한 길이 (문장 끊김 방지)
python scripts/train.py --max_new_tokens 200

# ---------------------- 매우 긴 요약 (300 토큰) ---------------------- #
# 복잡한 대화의 상세한 요약을 위한 긴 길이
python scripts/train.py --max_new_tokens 300
```
...
- **권장값: 200** (한국어는 영어보다 2-3배 많은 토큰 필요)

#### `--min_new_tokens` (최소 생성 토큰 수) ⭐ 신규 추가
```bash
# ==================== 최소 생성 토큰 수 설정 ==================== #
