...
# ---------------------- 생성 파라미터 딕셔너리 ---------------------- #
default_config = {
    'max_new_tokens': 200,          # ✅ 생성할 최대 토큰 수 (한국어 권장: 200)
    ...
    'repetition_penalty': 1.2,      # 반복 페널티 (반복 억제)
}
```
...
# ------------------------------- 추론 생성 파라미터 설정 ------------------------------- #
inference:
  generate_max_new_tokens: 200                          # ✅ 생성할 최대 토큰 수 (권장)
...
summaries = predictor.predict_batch(
    dialogues,                  # 대화 리스트
    batch_size=16,              # 배치 크기
    max_new_tokens=200,         # ✅ 오버라이드: 최대 200토큰 생성
    min_new_tokens=30,          # ✅ 오버라이드: 최소 30토큰 보장
    num_beams=8,                # 오버라이드: Beam 개수 증가
    repetition_penalty=1.3      # 오버라이드: 반복 억제 강화
)
```
...
| 파라미터 | 권장값 | 설명 |
|---------|-------|------|
| `max_new_tokens` ⭐ | **200** | **생성할 최대 토큰 수** (한국어는 영어보다 2-3배 필요) |
| `min_new_tokens` ⭐ | **30** | **생성할 최소 토큰 수** (문장 끊김 방지) |
| `num_beams` | 5 | Beam search 빔 개수 (높을수록 품질↑, 속도↓) |
| `repetition_penalty` | 1.2 | 반복 억제 강도 (1.0: 억제 없음, >1: 반복 억제) |
...
optimal_config = {
    # ✅ 길이 제어
    'max_new_tokens': 200,      # 한국어는 200토큰 권장
