...
│   │   │   ├── evaluate_dev.py     # Dev 평가
│   │   │   ├── topic_clustering.py # 토픽 클러스터링
│   │   │   └── ...                 # 기타 분석 스크립트
│   │   └── utils/                  # 유틸리티 함수
│   │       ├── config.py           # Config 파싱
│   │       ├── logger.py           # 로깅
│   │       └── metrics.py          # ROUGE 계산
│   ├── archive/                    # 이전 실험 기록
│   │   ├── experiments/            # 실험 스크립트 아카이브
│   │   ├── notebooks/              # 실험 노트북 아카이브
│   │   └── tests/                  # 테스트 코드
│   ├── examples/
│   │   └── example_usage.sh        # 사용 예시
│   ├── baseline.ipynb              # 공식 Baseline (36.12점)
│   ├── baseline_modular.ipynb      # 모듈화 버전
│   ├── eda_notebook.ipynb          # 탐색적 데이터 분석
│   ├── config.yaml                 # 모든 실험 설정
│   └── requirements.txt            # Python 의존성
├── docs/                           # 포트폴리오 문서
│   ├── COMPETITION_FINAL_REPORT.md # 최종 보고서
│   ├── EXPERIMENT_LOG.md           # 실험 로그
│   ├── LESSONS_LEARNED.md          # 교훈 정리
│   └── topic_clustering.md         # 토픽 분석
├── .gitignore                      # Git 제외 목록
└── README.md                       # 프로젝트 메인 문서

참고: data/, submission/, logs/, wandb/ 등은 .gitignore에 의해 GitHub에 포함되지 않음
```
